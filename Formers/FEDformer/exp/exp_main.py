...
#                         raise Exception('aaa')
    ...
    def test(self, setting, test=0):
        data_path = os.path.join(setting.root_path, setting.data_path)
        save_path = 'test_results/new_collection'
        test_data, test_loader = self._get_data(flag='test')
        if test:
            print('loading model')
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                gt = np.concatenate((gt, true[:, -1, f_dim:]), axis=0)
                pd = np.concatenate((pd, pred[:, -1, f_dim:]), axis=0)
                # gt = np.concatenate((gt, true[0, :-1, -1]), axis=0)
                # pd = np.concatenate((pd, pred[0, :-1, -1]), axis=0)
            visual(gt, pd, os.path.join(folder_path, str('test') + '.pdf'),data_path=data_path,seq_len=setting.seq_len,save_path=save_path,model_name=setting.model)
                # if i % 20 == 0:
                #     input = batch_x.detach().cpu().numpy()
                #     gt = np.concatenate((input[0, :, -1], true[0, :, -1]), axis=0)
                #     pd = np.concatenate((input[0, :, -1], pred[0, :, -1]), axis=0)
                #     visual(gt, pd, os.path.join(folder_path, str(i) + '.pdf'),data_path=data_path)
