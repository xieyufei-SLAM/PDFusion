...
parser.add_argument('--model_list', type = list, default=['PatchTST_TIA'], help='li-ion version')
...
# data loader
parser.add_argument('--data', type=str, default='custom', help='dataset type:custom_NoCorrection, custom')
parser.add_argument('--root_path', type=str, default=r'F:\LSTF_Projects\PatchTST_supervised\data\Li_ion', help='root path of the data file')
parser.add_argument('--data_path', type=str, default=r'1号lpf电池0.5，1充放电实验.xlsx', help='data file')
...
parser.add_argument('--if_incremental', default=False, help='if use IL to domain transfer')
parser.add_argument('--if_correction', default=False, help='if use Correction to domain transfer')
parser.add_argument('--source_path', default='1号lpf电池0.5，0.5充放电曲线.xlsx', help='Domain_path')
parser.add_argument('--target_path', default='1号ncm电池0.5，0.5充放电实验.xlsx', help='Domain_path')
parser.add_argument('--li_ion', default='lpf', help='li-ion version')
...
if __name__ == '__main__':
    ...
    for x in args.model_list:
        args.model = x
        for main_dir in ['data/Li_ion/DCD/lpf','data/Li_ion/hppc/lpf','data/Li_ion/DCD/ncm','data/Li_ion/hppc/ncm']:
    # main_dir = 'data/Li_ion/DCD/ncm'
            args.li_ion = main_dir.split('/')[-1]
            args.root_path = main_dir
            rmse_soc, mae_soc, mape_soc, r2_soc = [], [], [], []
            rmse_soe, mae_soe, mape_soe, r2_soe = [], [], [], []

            for data in os.listdir(main_dir):
                ...
                if args.is_training:
                    for ii in range(args.itr):
                        ...
                        if args.if_incremental:
                            ...
                            rmse, mae, mape, R2, rmse1, mae1, mape1, R21 = exp.test(setting, test=1, Domain_index='target')
                        else:
                ...
                rmse_soc.append(rmse)
                mae_soc.append(mae)
                mape_soc.append(mape)
                r2_soc.append(R2)

                rmse_soe.append(rmse1)
                mae_soe.append(mae1)
                mape_soe.append(mape1)
                r2_soe.append(R21)
            print(f'{args.li_ion}:average' + '>'*10)

            print('SOC:   rmse:{}, mae:{} mape: {} R2:{}'.format(np.array(rmse_soc).mean(), np.array(mae_soc).mean(), np.array(mape_soc).mean(), np.array(r2_soc).mean()))
            print('SOE:   rmse:{}, mae:{} mape: {} R2:{}'.format(np.array(rmse_soe).mean(), np.array(mae_soe).mean(), np.array(mape_soe).mean(), np.array(r2_soe).mean()))

            # print(f'{args.li_ion}:average of SOC and SOE' + '>' * 10)
            print(f'\033[92m{args.model} {args.li_ion} {main_dir}:average of SOC and SOE\033[0m' + '\033[92m>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\033[0m')

            print('SOX:   rmse:{}, mae:{} mape: {} R2:{}'.format((np.array(rmse_soe).mean() + np.array(rmse_soc).mean())/2,
                                                                 (np.array(mae_soe).mean() + np.array(mae_soc).mean())/2,
                                                                 (np.array(mape_soe).mean() + np.array(mape_soc).mean())/2,
                                                                 (np.array(r2_soe).mean() + np.array(r2_soc).mean())/2))
