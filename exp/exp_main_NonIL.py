...
class Exp_Main_IL(Exp_Basic):
    ...
    def _build_model(self):
        ...
        self.DomainAdaptor = Domain_Adaptor(5,16,9)
    ...
    def test(self, setting, test=0, Domain_index='source'):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                gt = np.concatenate((gt, true[:, -1, -1]), axis=0)
                pd = np.concatenate((pd, pred[:, -1, -1]), axis=0)
                # gt = np.concatenate((gt, true[0, :-1, -1]), axis=0)
                # pd = np.concatenate((pd, pred[0, :-1, -1]), axis=0)

            visual(gt, pd, os.path.join(folder_path, str('test') + '.pdf'))
                # if i % 20 == 0:
                #     input = batch_x.detach().cpu().numpy()
