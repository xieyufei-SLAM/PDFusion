...
from utils.tools import EarlyStopping, adjust_learning_rate, visual, visual_No_Correction, test_params_flop
...
class Exp_Main(Exp_Basic):
    ...
    def _get_data(self, flag):
        data_set, data_loader = data_provider(self.args, flag, Domain_index=None)
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            ...
            if self.args.if_correction:
                save_path = f'test_results/new_collection_Correction/{self.args.model}'
            else:
                save_path = f'test_results/new_collection_calce/{self.args.model}'
            if self.args.if_correction:
                ...
                rmse, mae, mape, R2, rmse1, mae1, mape1, R21 = visual_No_Correction(gt, pd, os.path.join(folder_path, str('test') + '.pdf'), data_path=data_path,
                       seq_len=self.args.seq_len, save_path=save_path, excel_name=(self.args.data_path.split('.xls')[0]+'.xlsx'))
