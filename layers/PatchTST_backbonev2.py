...
class PatchTST_backbone(nn.Module):
    def __init__(self, c_in: int, context_window: int, target_window: int, patch_len: int, stride: int,
                 ...
                 subtract_last=False,
                 verbose: bool = False,Incremental_learning=False,c_out=2,**kwargs):
        ...
        self.backbone = TSTiEncoder(c_in, patch_num=patch_num, patch_len=patch_len, max_seq_len=max_seq_len,
                                    ...
                                    pe=pe, learn_pe=learn_pe, verbose=verbose,incremental_l=Incremental_learning,**kwargs)
        ...
        if self.pretrain_head:
            ...
            self.head = Flatten_Head(self.individual, self.c_out, self.head_nf, target_window,
                                     head_dropout=head_dropout)

        from layers.Embed import DataEmbedding_Feature_Time
        self.Data_embedding = DataEmbedding_Feature_Time(context_window,d_model)
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        ...
        z,inners = self.backbone(z)  # torch.Size([128, 7, 512, 12] # z: [bs x nvars x d_model x patch_num]
...
class TSTiEncoder(nn.Module):  # i means channel-independent
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,
                 ...
                 pe='zeros', learn_pe=True, verbose=False, incremental_l=False, **kwargs):
        super().__init__()
        ...
        self.encoder = TSTEncoder(q_len, d_model, n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                  ...
                                  store_attn=store_attn,incremental_l=incremental_l)

    def forward(self, x) -> (Tensor,dict):  # x: [bs x nvars x patch_len x patch_num]
        ...
        # TODO Encoder：这里获取了中间层的信息，包括注意力分数的图
        z,inners = self.encoder(
...
class TSTEncoder(nn.Module):
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=None,
                 ...
                 res_attention=False, n_layers=1, pre_norm=False, store_attn=False,incremental_l=False):
        super().__init__()
        ...
        self.incremental_l = incremental_l
    ...
    def forward(self, src: Tensor, key_padding_mask: Optional[Tensor] = None, attn_mask: Optional[Tensor] = None):
        ...
        freq_scores = None
        # 输入大小：[128*7, 12, 512]
        #TODO 如果是增量学习，则获取中间特征图信息
        Inner_results = {
            'inners_out':torch.tensor([],device=src.device),
            'score':torch.tensor([],device=src.device),
            'freq_score':torch.tensor([],device=src.device)
        }
        if self.res_attention:
            for i,mod in enumerate(self.layers):
                output, scores, freq_scores = mod(output, prev=scores, prev_freq=freq_scores,
                                                                      key_padding_mask=key_padding_mask,
                                                                      attn_mask=attn_mask)
                if (self.incremental_l is True) and (i == (len(self.layers)-1)):
                    Inner_results['inners_out'] = torch.concat((Inner_results['inners_out'],output),dim=-1)
                    Inner_results['score'] = torch.concat((Inner_results['score'],scores),dim=-1)
                    Inner_results['freq_score'] = torch.concat((Inner_results['freq_score'],freq_scores.to(src.device)),dim=-1)

                    # Inner_results['inners_out'].append(output)
                    # Inner_results['score'].append(scores)
                    # Inner_results['freq_score'].append(freq_scores)
            return output, Inner_results
        else:
            for mod in self.layers:
                output = mod(output, key_padding_mask=key_padding_mask, attn_mask=attn_mask)
                if self.incremental_l is True:
                    Inner_results['inners_out'].append(output)
            return output, Inner_results
...
class TSTEncoderLayer(nn.Module):
    ...
    def forward(self, src: Tensor, prev: Optional[Tensor] = None, prev_freq: Optional[Tensor] = None,
        ...
        if self.res_attention:
            return src, attn, freq_attn #返回softmax等归一化之后的注意力向量
        else:
            return src


# TODO 改进为趋势信息交互注意力机制(包括FFT和时域频域交互序列)
class _MultiheadAttention(nn.Module):
    ...
    def forward(self, Q: Tensor, K: Optional[Tensor] = None, V: Optional[Tensor] = None, prev: Optional[Tensor] = None,
        ...
        if self.res_attention:
            ...
            output, attn_weights, attn_socres_freqw, attn_scores, attn_socres_freq = self.ITFA(q_s, k_s, v_s, key_padding_mask=key_padding_mask,
                                                                attn_mask=attn_mask)
...
class InteractiveTimeFeqAttention(nn.Module):
    ...
    def __init__(self, d_model, n_heads, len_q, len_kv, attn_dropout=0., res_attention=False, lsa=False,
        ...
        # 设定频域所需的特征长度
        self.index_q = len_q//2
        self.index_kv = len_q//2
        ...
        self.Trifuion = nn.Sequential(
            nn.Dropout(0.1),
            nn.Linear(d_model // n_heads, d_model // n_heads),
            nn.LayerNorm(d_model // n_heads)
        )
        # 频率域的复数乘法所用权重
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor, prev: Optional[Tensor] = None,
        ...
        # out = self.Trifuion((freq_out + time_out))
